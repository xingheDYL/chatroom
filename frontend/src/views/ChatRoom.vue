  ...
  <div class="chat-room">
    ...
    <div class="main-chat">
      ...
      <template v-else>
        ...
        <div ref="messagesContainer" class="messages-container">
          ...
          >
            <div class="message-avatar" @click="startPrivateChat(message.senderId, message.senderName)" :class="{ clickable: message.senderId !== currentUserId }">
            ...
            <div class="message-content">
              ...
              <div class="message-text">{{ message.content }}<small v-if="!message.content" style="color:red">(空)</small></div>
            </div>
          </div>
        </div>
    ...
    <div class="users-panel">
      ...
      <div class="users-list">
        <div v-for="member in members" :key="member.id" class="user-item" @click="startPrivateChat(member.id, member.username)" :class="{ clickable: member.id !== currentUserId }">
          <el-avatar :src="member.avatar">{{ member.username ? member.username[0] : '?' }}</el-avatar>
          <span class="user-name">{{ member.username }}</span>
          <span v-if="member.status" class="user-status" :class="member.status">
